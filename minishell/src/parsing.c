...
/*   Updated: 2025/03/17 13:02:43 by mkhlouf          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	line_split(t_data *data)
{
	...
	while (data->cleaned_line[i])
		{
			if(data->cleaned_line[i] == 27 || data->cleaned_line[i] == 22)
			{
				if (skip == false)
					skip = true;
				else
				...
				if (skip == false)
				{
					data->tokens[j].data = ft_substr(data->cleaned_line, start, i - start +1);
					start = i + 1;
					printf("token #%s#\n", data->tokens[j].data);
					j++;
				}
			}
			if ((data->cleaned_line[i] == ' ' || data->cleaned_line[i + 1] == '\0') && !skip)
			{
