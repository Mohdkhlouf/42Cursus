...
/*   Updated: 2025/01/02 17:36:56 by mkhlouf          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	read_map_to_array(int fd, char *row, t_game *game)
{
	...
	while (i < game->map_orginal.map_height && bytes_read > 0)
	{
		...
		if (bytes_read > 129600 || game->map_orginal.map_height > 35
			|| game->map_orginal.map_width > 60)
...
void	handle_new_line(unsigned int *first_row_size, t_game *game,
		char *buffer)
{
	if (*buffer == '\n')
	{
		...
		{
			free(buffer);
			print_error_and_exit("Map is not rectangle shape", game);
		}
